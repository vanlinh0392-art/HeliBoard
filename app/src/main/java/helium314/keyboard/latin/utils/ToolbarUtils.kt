...
fun getCodeForToolbarKey(key: ToolbarKey) = if (key == VOICE) -233 else Settings.getInstance().getCustomToolbarKeyCode(key) ?: when (key) {
    VOICE -> -233 // KeyCode.VOICE_INPUT
    ...
    EMOJI -> KeyCode.EMOJI
    STICKERS -> KeyCode.SHOW_STICKERS
...
enum class ToolbarKey {
    ...
    INCOGNITO, AUTOCORRECT, CLEAR_CLIPBOARD, CLOSE_HISTORY, EMOJI, STICKERS, LEFT, RIGHT, UP, DOWN, WORD_LEFT, WORD_RIGHT,
...
val defaultToolbarPref by lazy {
    val default = listOf(SETTINGS, VOICE, CLIPBOARD, UNDO, REDO, SELECT_WORD, COPY, PASTE, LEFT, RIGHT, STICKERS)
